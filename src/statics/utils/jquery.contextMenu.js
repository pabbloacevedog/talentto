...
(function ($, window, document, undefined) {
	...
	$.widget("moogle.contextmenu", {
		...
		_create: function () {
			...
			if (opts.preventSelect) {
				...
				targetId = (
					$(this.element).is(document) ? $("body") : this.element
				)
					.uniqueId()
					.attr("id");
				cssText =
					...
					"}";
				this.$headStyle = $(
					"<style scoped class='moogle-contextmenu-style' />"
				)
					.prop("type", "text/css")
					.appendTo("head");
				...
				if (supportSelectstart) {
					this.element.delegate(
						opts.delegate,
						"selectstart" + this.eventNamespace,
						function (event) {
			...
			this.$menu
				...
				.menu({
					...
					select: $.proxy(function (event, ui) {
						...
						var retval,
							isParent =
								ui.item.has(">a[aria-haspopup='true']").length >
								0,
							$a = ui.item.find(">a"),
		...
		_closeMenu: function (immediately) {
			...
			this.$menu.unbind("contextmenu" + this.eventNamespace);
		...
		_getMenuEntry: function (cmd, wantLi) {
			var $entry = this.$menu.find(
				"li a[href=#" + normCommand(cmd) + "]"
			);
		...
		enableEntry: function (cmd, flag) {
			this._getMenuEntry(cmd, true).toggleClass(
				"ui-state-disabled",
				flag === false
			);
		},
	...
	$.extend($.moogle.contextmenu, {
		...
		createMenuMarkup: function (options, $parentUl) {
			var i, menu, $ul, $li;
			if ($parentUl == null) {
				$parentUl = $("<ul class='ui-helper-hidden' />").appendTo(
					"body"
				);
			}
